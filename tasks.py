...
def make_code_task_analysis(
    ...
    backend_task = Task(
        description=(
            ...
            "  NIVEAU 1 — Lecture ciblée (toujours commencer ici) :\n"
            "    → Lire uniquement les lignes indiquées par le Manager (start_line + line_count).\n"
            "    → Maximum 50 lignes par lecture. Jamais tout le fichier d'un coup.\n\n"
            "  NIVEAU 2 — Remontée au caller (si dépendance externe détectée) :\n"
            "    → Si la fonction utilise une variable/fonction définie ailleurs dans le fichier :\n"
            "       a) Identifier le nom exact (ex: `chat_rate_limiter`, `supabase_anon`)\n"
            "       b) Faire une 2ème lecture ciblée de ~15 lignes autour de sa définition\n"
            "       c) Maximum 2 remontées — si toujours bloqué → documenter le blocage\n"
            "    → Coût d'une remontée : ~15 lignes = négligeable vs lire 800 lignes\n\n"
            "  NIVEAU 3 — Lecture de structure (en dernier recours seulement) :\n"
            "    → Lire les 30 premières lignes du fichier (imports + globals uniquement)\n"
